...
/*****************************Scrap Amazon Product Attributes**********************************/
			...
			if($c != 0){
				...
				$suppliers_num = (int)$supplier_num[1];
...
/********************************Offer Listing**************************************************************/
				...
				$amazon3->load($amz3);
				////echo $amz3;
				$suppliers = $amazon3->find('div[class="a-row a-spacing-mini olpOffer"]');
				...
				//echo $arrnonfba[0].'</br>';
				//echo $arrfba[0].'</br>';	
				if($fba == 0 && $suppliers_num > 10){
					$inhref = $nhref.'&startIndex=10';
					$curl = curl_init(); 
					curl_setopt($curl,CURLOPT_URL, $inhref);
					curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
					curl_setopt($curl, CURLOPT_COOKIESESSION, FALSE);
					curl_setopt($curl, CURLOPT_FOLLOWLOCATION, TRUE);
					curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
					...
					$amz4=curl_exec($curl);
					$amazon4->load($amz4);
					$suppliers = $amazon4->find('div[class="a-row a-spacing-mini olpOffer"]');
				$offers = $amazon4->find('span[class="a-size-large a-color-price olpOfferPrice a-text-bold"]');
				$seller_new = $amazon4->find('h3[class="a-spacing-small olpCondition"]');
				$seller_name = $amazon4->find('p[class="a-spacing-mini olpSellerName"]');
				//$seller_cname = $seller_name->children[0];
				////echo sizeof($offers).'</br>';
				////echo sizeof($seller_new).'</br>';
				////echo sizeof($seller_name).'</br>';
